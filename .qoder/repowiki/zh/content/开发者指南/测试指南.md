...
## 更新摘要
**变更内容**
- 新增自定义索引名称（indexName）功能的测试覆盖
- 更新内容扫描模块测试策略，涵盖自定义索引识别、回退行为和大小写不敏感匹配
- 增强 EPUB 导航结构测试，验证索引文件优先级和目录跳转逻辑
...
**更新** 新增对自定义索引名称（indexName）功能的全面测试覆盖，包括索引文件识别、回退行为和大小写不敏感匹配等场景。
...
graph TB
subgraph "测试层"
T1["test/epub-index-navigation.test.cjs<br/>60行新测试代码"]
...
subgraph "服务层"
S1["services/contentScanner.ts<br/>支持自定义indexName"]
S2["services/epubService.ts"]
S3["services/outputResolver.ts"]
S4["services/metadata.ts<br/>新增indexName字段"]
...
- [test/epub-index-navigation.test.cjs:1-200](file://test/epub-index-navigation.test.cjs#L1-L200)
...
## 核心组件
- 内容扫描（contentScanner）：负责扫描目录、解析数字前缀排序、识别 index 文件、构建树状与线性节点结构，并应用 .t2eignore 规则与 __t2e.data 系统保留目录策略。**更新** 支持自定义索引名称参数，通过 `indexName` 参数控制索引文件识别逻辑。
...
- 元数据（metadata）：读取/写入 metadata.yml，格式化文件名，提供展示标题与作者。**更新** 新增 `indexName` 字段，默认值为 'index'。
...
**章节来源**
- [src/services/contentScanner.ts:51-350](file://src/services/contentScanner.ts#L51-L350)
...
- [src/services/metadata.ts:41-160](file://src/services/metadata.ts#L41-L160)
...
META-->>CMD : "元数据对象(含indexName)"
CMD->>CS : "scanContentTree(目标目录, metadata.indexName)"
CS-->>CMD : "内容树与线性文件列表(含自定义索引)"
...
- [src/services/outputResolver.ts:15-42](file://src/services/outputResolver.ts#L15-L42)
- [src/services/metadata.ts:41-62](file://src/services/metadata.ts#L41-L62)
...
### 内容扫描（contentScanner）
- 关键职责：扫描目录、解析数字前缀排序、识别 index 文件、构建树与拍平文件列表、应用 .t2eignore 与 __t2e.data 策略。**更新** 支持自定义索引名称参数，通过 `isIndexDisplayName` 函数实现大小写不敏感的索引文件识别。
- 测试要点：
...
Ext --> |否| Skip
Ext --> |是| CheckIndex["检查是否为索引文件<br/>isIndexDisplayName(displayName, indexName)"]
CheckIndex --> BuildFile["构建文件节点含 display/order/isIndexFile"]
...
**章节来源**
- [src/services/contentScanner.ts:51-350](file://src/services/contentScanner.ts#L51-L350)
- [test/epub-index-navigation.test.cjs:74-200](file://test/epub-index-navigation.test.cjs#L74-L200)
...
### 自定义索引名称功能测试（新增）
- 关键职责：验证自定义索引名称功能的正确性，包括索引文件识别、回退行为和大小写不敏感匹配。
- 测试要点：
  - 配置自定义 indexName 后，所有匹配格式均被识别为 index 文件
  - 配置自定义 indexName 后，原 index 文件不再被识别为 index 文件
  - indexName 为空字符串时回退到默认 index 识别行为
  - indexName 匹配大小写不敏感
...
**章节来源**
- [test/epub-index-navigation.test.cjs:141-200](file://test/epub-index-navigation.test.cjs#L141-L200)
- [src/services/contentScanner.ts:255-258](file://src/services/contentScanner.ts#L255-L258)

## 依赖关系分析
- 命令层依赖服务层：generateEpub 依赖 contentScanner、epubService、outputResolver、metadata、folderMatcher。**更新** contentScanner 现在接收 metadata.indexName 参数。
- 服务层内部耦合：
  - contentScanner 依赖 t2eIgnore 与 folderMatcher，**更新** 新增对 indexName 参数的处理
...
```mermaid
graph LR
CMD["generateEpub.ts"] --> CS["contentScanner.ts<br/>支持indexName参数"]
CMD --> EP["epubService.ts"]
CMD --> OR["outputResolver.ts"]
CMD --> META["metadata.ts<br/>新增indexName字段"]
...
- 常见错误与定位：
  ...
  - **新增** "自定义索引文件未被识别"：检查 metadata.yml 中 indexName 配置、文件名格式和大小写设置。
- 调试技巧：
  - 使用临时目录与 JSZip 读取 EPUB 内容，断言关键文件存在与内容片段。
  - 在命令层 withProgress 中观察阶段性消息，定位卡顿环节。
  - 对照测试用例的目录结构与断言，逐步缩小问题范围。
  - **新增** 通过测试用例验证自定义索引名称功能的各个场景。

**章节来源**
- [src/services/epubService.ts:156-158](file://src/services/epubService.ts#L156-L158)
- [src/services/epubService.ts:604-633](file://src/services/epubService.ts#L604-L633)
- [src/services/outputResolver.ts:15-42](file://src/services/outputResolver.ts#L15-L42)
- [test/epub-index-navigation.test.cjs:39-200](file://test/epub-index-navigation.test.cjs#L39-L200)

## 结论
本指南提供了针对 Folder2EPUB 的测试策略与实施路径：以现有测试为起点，围绕内容扫描、EPUB 构建与输出解析三大核心模块设计单元与集成测试；通过临时目录与 EPUB 内容断言，确保关键流程的正确性与稳定性；结合进度提示与错误消息，提升用户体验与可观测性。**更新** 新增的自定义索引名称功能测试覆盖了索引文件识别、回退行为和大小写不敏感匹配等关键场景，进一步增强了系统的灵活性和用户自定义能力。建议在 CI/CD 中引入自动化测试与覆盖率统计，持续保障质量。
...
- 测试用例设计：
  ...
  - **新增** 自定义索引名称场景：覆盖自定义名称识别、回退行为、大小写不敏感匹配等。
- 断言使用：
...
- 质量标准：
  ...
  - 测试用例命名清晰，断言信息可读性强。
  - 避免测试间相互依赖，每个测试独立可运行。
  - **新增** 自定义索引名称功能测试覆盖率达到 90% 以上。

### 关键功能模块测试策略
- 内容扫描：
  - 设计多层级目录与 index 回退场景，断言 firstFile/indexFile 选择。
  - 验证数字前缀与中文排序一致性。
  - **新增** 验证自定义索引名称识别逻辑，包括大小写不敏感匹配。
- EPUB 构建：
...
**章节来源**
- [src/services/contentScanner.ts:107-170](file://src/services/contentScanner.ts#L107-L170)
- [src/services/epubService.ts:226-260](file://src/services/epubService.ts#L226-L260)
- [src/services/outputResolver.ts:50-89](file://src/services/outputResolver.ts#L50-L89)
- [test/epub-index-navigation.test.cjs:141-200](file://test/epub-index-navigation.test.cjs#L141-L200)
...
- 测试数据：
  ...
  - **新增** 准备多种索引文件命名格式的测试数据，包括自定义名称、大小写变化等。
- 模拟对象：
