...
- [example/index-demo/0010_前言.md](file://example/index-demo/0010_前言.md)
- [example/index-demo/0020_正文/_preface.md](file://example/index-demo/0020_正文/_preface.md)
- [example/index-demo/0020_正文/0010_第一章.md](file://example/index-demo/0020_正文/0010_第一章.md)
- [example/index-demo/0020_正文/0020_第二章.txt](file://example/index-demo/0020_正文/0020_第二章.txt)
- [example/index-demo/0020_正文/0030_小节/_preface.md](file://example/index-demo/0020_正文/0030_小节/_preface.md)
- [example/index-demo/0030_附录/__index.md](file://example/index-demo/0030_附录/__index.md)
- [example/index-demo/0040_默认索引/_index.md](file://example/index-demo/0040_默认索引/_index.md)
- [example/index-demo/0050_混合测试/preface.md](file://example/index-demo/0050_混合测试/preface.md)
- [example/index-demo/0050_混合测试/_preface.txt](file://example/index-demo/0050_混合测试/_preface.txt)
- [example/index-demo/0050_混合测试/1234_preface.md](file://example/index-demo/0050_混合测试/1234_preface.md)
- [example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/_preface.md](file://example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/_preface.md)
- [example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/0010_内容.md](file://example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/0010_内容.md)
- [example/index-demo/0070_散文件/0010_散章一.md](file://example/index-demo/0070_散文件/0010_散章一.md)
- [example/index-demo/0070_散文件/0020_散章二.txt](file://example/index-demo/0070_散文件/0020_散章二.txt)
...
## 更新摘要
**变更内容**
- 新增 indexName 功能详解，展示自定义索引文件命名约定的实际应用场景
- 更新索引文件处理最佳实践，包括不同命名约定、嵌套目录、混合格式等场景
- 完善配置驱动的索引文件识别机制说明
- 增强内容组织建议中的索引文件命名策略指导
...
A --> C["索引演示<br/>index-demo/"]
A --> D["空目录示例<br/>empty-folder/"]
B --> E["元数据目录<br/>__t2e.data/"]
C --> F["配置驱动索引<br/>indexName 功能"]
E --> G["元数据文件<br/>metadata.yml"]
E --> H["封面文件<br/>cover.*示例为 webp"]
B --> I["忽略规则<br/>.t2eignore"]
B --> J["章节与子目录<br/>00100__章节 1.txt 等"]
B --> K["索引文件示例<br/>_index.* 与 0000__index.*"]
C --> L["多场景索引测试<br/>不同命名约定与格式"]
A --> M["全局输出配置<br/>__epub.yml"]
```

图中展示了示例项目的典型布局：元数据目录、章节文件、索引文件与忽略规则的协同关系，以及全局输出配置对生成位置的影响。新增的 index-demo 目录专门演示了 indexName 功能的各种应用场景。

**图表来源**
- [example/index-demo/__t2e.data/metadata.yml:1-8](file://example/index-demo/__t2e.data/metadata.yml#L1-L8)
- [example/index-demo/0010_前言.md:1-10](file://example/index-demo/0010_前言.md#L1-L10)
- [example/index-demo/0020_正文/_preface.md:1-7](file://example/index-demo/0020_正文/_preface.md#L1-L7)
- [example/index-demo/0030_附录/__index.md:1-12](file://example/index-demo/0030_附录/__index.md#L1-L12)
- [example/index-demo/0040_默认索引/_index.md:1-11](file://example/index-demo/0040_默认索引/_index.md#L1-L11)
...
**新增功能**：索引文件处理功能现已完善，包括 `stripTopMarkdownHeading` 函数的实现与使用场景。

- **索引文件识别**：支持 `_index.*` 和 `0000__index.*` 等命名模式，自动识别目录内的索引文件。
- **目录跳转机制**：当目录包含索引文件时，该目录优先跳转至索引文件，索引文件内容直接作为分组标题下的内容。
- **标题规范化处理**：索引文件内容中的最高层级标题会被自动移除，避免与外层标题冲突。
- **stripTopMarkdownHeading 函数**：专门用于移除 Markdown 内容中的最高层级标题，确保索引文件内容与上级标题层级协调。
...
B --> C{"找到索引文件？"}
C --> |是| D["读取索引文件内容"]
D --> E["stripTopMarkdownHeading(content)"]
E --> F["移除最高层级标题"]
F --> G["输出索引内容到分组标题下"]
C --> |否| H["使用首个文件作为目录跳转"]
G --> I["继续处理子节点"]
...
**图表来源**
- [src/services/markdownService.ts:97-104](file://src/services/markdownService.ts#L97-L104)
- [src/services/markdownService.ts:167-180](file://src/services/markdownService.ts#L167-L180)

**章节来源**
- [src/services/markdownService.ts:93-180](file://src/services/markdownService.ts#L93-L180)
- [src/services/contentScanner.ts:113-115](file://src/services/contentScanner.ts#L113-L115)

### Markdown 合并与处理
- 标题层级调整：根目录文件内容中的子标题自动提升两级，避免与主标题冲突。
- **索引文件处理增强**：目录内的 index 文件内容直接作为分组标题下的内容，不重复输出章节标题。使用 `stripTopMarkdownHeading` 函数移除索引文件内容中的最高层级标题。
...
E --> |目录| G["输出分组标题"]
G --> H["检查索引文件"]
H --> I["stripTopMarkdownHeading(content)"]
I --> J["输出索引文件内容"]
J --> K["递归处理子节点"]
F --> L["处理 frontmatter 和图片"]
L --> M["调整标题层级"]
M --> N["清理空行"]
N --> O["追加到输出"]
K --> D
...
- **索引文件未生效**
  - 现象：目录未优先跳转到 index。
  - 处理：确保 index 文件命名符合规范（如 _index.* 或 0000__index.*），且位于目录内。检查索引文件内容是否包含最高层级标题，这些标题会被自动移除。
- **indexName 配置问题**
  - 现象：自定义索引文件命名未按预期工作。
  - 处理：检查 metadata.yml 中的 indexName 配置，确认其值与实际文件名匹配。例如 `indexName: preface` 会使名为 `preface` 的文件被识别为索引文件。
- 合并 Markdown 失败
...
- [src/services/outputResolver.ts:15-42](file://src/services/outputResolver.ts#L15-L42)
- [src/services/contentScanner.ts:113-141](file://src/services/contentScanner.ts#L113-L141)
- [example/index-demo/__t2e.data/metadata.yml:5-6](file://example/index-demo/__t2e.data/metadata.yml#L5-L6)
...
- 在复杂内容场景下保持可维护性与可扩展性
- 在性能与稳定性之间取得平衡
- **充分利用索引文件功能，提升内容组织的灵活性与可读性**
- **掌握 indexName 配置驱动的索引文件命名策略，实现更灵活的内容组织**
...
- **索引文件使用增强**
  ...
  - **stripTopMarkdownHeading 函数会自动移除索引文件内容中的最高层级标题，确保与上级标题层级协调**
...
**新增功能**：详细介绍索引文件处理的技术实现与最佳实践。

#### stripTopMarkdownHeading 函数详解
- **功能目的**：移除 Markdown 内容中的最高层级标题，防止与外层标题冲突。
- **实现原理**：逐行扫描内容，跳过空行，找到第一个标题后移除该标题及其后的空行。
- **使用场景**：主要用于索引文件内容处理，确保索引文件内容与上级目录标题层级协调。

#### 索引文件处理流程
1. **索引文件识别**：扫描目录节点，查找 `isIndexFile` 为 true 的文件。
2. **内容读取**：读取索引文件的原始内容。
3. **标题处理**：调用 `stripTopMarkdownHeading` 移除最高层级标题。
4. **内容输出**：将处理后的内容直接输出到分组标题下，不重复输出章节标题。

#### 最佳实践建议
- **索引文件命名**：使用 `_index.md` 或 `0000__index.md` 等标准命名。
- **内容组织**：索引文件应包含该目录的概述性内容，避免重复章节标题。
- **标题层级**：索引文件内容应从二级标题开始，避免与目录标题冲突。

### indexName 功能详解

**新增功能**：详细介绍 indexName 配置驱动的索引文件命名约定。

#### indexName 配置机制
- **配置位置**：在 metadata.yml 中设置 `indexName` 字段。
- **功能作用**：自定义索引文件的识别规则，使指定名称的文件被识别为目录入口。
- **默认行为**：未配置时使用传统的 `_index.*` 和 `0000__index.*` 命名约定。

#### 实际应用场景

**场景一：自定义索引文件命名**
- 配置：`indexName: preface`
- 行为：目录中名为 `preface` 的文件（如 `_preface.md`、`1234_preface.md`）被识别为索引文件
- 优势：更简洁的命名方式，便于记忆和使用

**场景二：混合命名约定**
- 目录同时存在 `index` 和 `preface` 文件
- 配置 `indexName: preface` 后：
  - `preface` 文件：作为索引文件识别
  - `index` 文件：作为普通章节处理
...
**场景四：混合格式支持**
- 同一目录中支持 `.md` 和 `.txt` 格式的索引文件
- 例如：`_preface.md` 和 `_preface.txt`

#### 命名约定最佳实践
- **简单命名**：`preface.md`（无前缀，最简洁）
- **带前缀命名**：`1234_preface.md`（数字前缀 + 名称）
- **下划线前缀**：`_preface.txt`（下划线前缀 + 扩展名）
- **组合命名**：`0000__preface.md`（数字 + 下划线 + 名称）
...
cover: ''
indexName: preface  # 关键配置
version: 1.0.0
```

**效果对比**：
- 传统方式：必须使用 `_index.md` 或 `0000__index.md`
- 自定义方式：使用 `preface` 作为索引文件标识符
- 兼容性：同时支持多种命名约定在同一项目中使用
...
- [src/services/contentScanner.ts:113-115](file://src/services/contentScanner.ts#L113-L115)
- [example/index-demo/__t2e.data/metadata.yml:5-6](file://example/index-demo/__t2e.data/metadata.yml#L5-L6)
- [example/index-demo/0010_前言.md:5-6](file://example/index-demo/0010_前言.md#L5-L6)
- [example/index-demo/0020_正文/_preface.md:5-6](file://example/index-demo/0020_正文/_preface.md#L5-L6)
- [example/index-demo/0030_附录/__index.md:5-6](file://example/index-demo/0030_附录/__index.md#L5-L6)
- [example/index-demo/0040_默认索引/_index.md:6-7](file://example/index-demo/0040_默认索引/_index.md#L6-L7)
- [example/index-demo/0050_混合测试/preface.md:5-6](file://example/index-demo/0050_混合测试/preface.md#L5-L6)
- [example/index-demo/0050_混合测试/1234_preface.md:14-15](file://example/index-demo/0050_混合测试/1234_preface.md#L14-L15)
- [example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/_preface.md:3-4](file://example/index-demo/0060_多层嵌套/0010_深层/0010_更深处/_preface.md#L3-L4)
- [example/index-demo/0070_散文件/0010_散章一.md:5-6](file://example/index-demo/0070_散文件/0010_散章一.md#L5-L6)