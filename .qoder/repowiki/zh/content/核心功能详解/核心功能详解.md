...
## 架构总览
下图展示了“生成 EPUB”命令的端到端流程，包括元数据校验、内容扫描、输出目录解析与 EPUB 构建。
...
- 渲染与资源处理：
  - Markdown 渲染与图片重写在内存中完成，建议控制单章节大小与图片数量。
  - 图片媒体类型映射与存在性校验在构建前完成，避免运行时错误。
- 排序与遍历：
...
建议在实际使用中结合 __epub.yml 的输出目录配置与 .t2eignore 的规则管理，以获得更稳定与可维护的生成体验。
