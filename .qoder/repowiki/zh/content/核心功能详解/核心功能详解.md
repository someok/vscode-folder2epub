...
## 更新摘要
**变更内容**
- 新增"生成合并 Markdown"命令功能，提供将目录内容合并为单个 Markdown 文件的能力
- 增强 markdownService 的标题层级处理能力，支持智能标题层级调整
- 版本从 1.1.4 升级到 1.2.0
- 更新架构图和流程图以反映新增功能
- 完善标题层级处理算法的详细说明
...
**更新** 新增"生成合并 Markdown"功能，提供将目录内容合并为单个 Markdown 文件的能力，并增强了内容处理的灵活性。版本升级至 1.2.0，包含更完善的标题层级处理和内容过滤机制。
...
**增强功能** 标题层级处理算法得到显著增强，支持智能标题规范化和层级调整：

#### 标题层级处理算法
- **规范化处理**：将内容中出现的最小标题层级作为基准，统一左移到从 # 开始
- **层级调整**：根据文件在输出中的章节层级，整体下移标题层级
- **代码块保护**：跳过 fenced code block 内的标题行，避免影响代码格式
- **最大层级限制**：确保标题层级不超过 6 级
...
H --> J
J --> K{"根目录文件?"}
K --> |是| L["normalizeMarkdownHeadings()"]
L --> M["adjustMarkdownHeadings(offset)"]
K --> |否| N["保持原样"]
M --> O["清理多余空行"]
N --> O
O --> P["生成章节标题基于层级"]
