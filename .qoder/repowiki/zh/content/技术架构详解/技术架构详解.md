...
- [markdownService.ts:1-137](file://src/services/markdownService.ts#L1-L137)
...
- 服务层
  ...
  - markdownService：Markdown 合并、标题层级调整、图片过滤、Frontmatter 处理。
...
- 关键接口
  ...
  - adjustMarkdownHeadings：调整 Markdown 内容中的标题层级，跳过代码块。
...
**章节来源**
- [markdownService.ts:30-137](file://src/services/markdownService.ts#L30-L137)
...
- 常见错误与定位
  ...
  - Markdown 合并失败：检查 Frontmatter 格式与文件编码。
- 错误处理策略
...
## 结论
本架构以命令层编排、服务层内聚为核心，结合 VS Code API 与本地化机制，实现了从目录扫描到 EPUB 3 打包以及 Markdown 合并的完整链路。新增的 Markdown 合并服务提供了与 EPUB 服务互补的功能，满足不同场景下的内容输出需求。模块化与低耦合的设计提升了可维护性与可扩展性；通过 .t2eignore 与 __epub.yml 的配置能力，满足不同场景的输出需求。建议在大规模内容场景下优化 I/O 与内存使用，并完善测试覆盖以保障稳定性。
