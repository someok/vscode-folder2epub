...
## 更新摘要
**变更内容**
- 新增 stripTopMarkdownHeading 功能说明，确保索引文件内容与父目录标题层次的协调
- 更新索引文件处理机制的技术细节，包括标题层级规范化和内容预处理
- 增强索引文件在 EPUB 导航中的表现，避免重复标题层级问题
...
- **新增功能**：stripTopMarkdownHeading 标题层级规范化，确保索引文件内容与父目录标题层次协调一致。
...
- **新增功能**：Markdown 内容处理与标题层级调整，包括 stripTopMarkdownHeading 功能，确保索引文件内容与父目录标题层次协调。
...
## 架构总览
索引文件处理贯穿"扫描—解析—选择—生成"的主流程。下图展示了从目录扫描到导航生成的关键步骤与组件交互，包括新增的标题层级规范化处理。
...
MS->>MS : "readFileContent(file, parentHeadingLevel, stripTopHeading)"
MS->>MS : "normalizeMarkdownHeadings(content)"
MS->>MS : "stripTopMarkdownHeading(content)"
MS->>MS : "adjustMarkdownHeadings(content, offset)"
...
## 结论
索引文件处理机制通过"数字前缀解析 + 展示名大小写不敏感 + 目录索引优先"的规则，在目录跳转与导航生成中实现了清晰、可控的行为。**新增的 stripTopMarkdownHeading 功能进一步增强了索引文件的处理能力，通过标题层级规范化确保索引文件内容与父目录标题层次协调一致，避免了重复标题层级的问题。**结合 .t2eignore 与元数据目录隔离，用户可以灵活地组织内容结构并精确控制扫描范围。测试用例进一步验证了在不同目录结构下索引文件的识别与导航表现，确保了功能的稳定性与可预期性。
